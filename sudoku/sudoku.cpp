  ...
  do{
        ...
        for(int v =1; v<10;v++){//faz os "filhos" do primeiro da pilha
            ...
            if(S.jogada_valida(J)){
                S.fazer_jogada(J); //Se a jogada é valida, então ela é feita.
                F.push(S);//Se a jogada é feita, é colocado outro Sudoku no topo da pilha
                S.x[i][j] = 0; //Esta posição recebe zero, para que seja testado outro tabuleiro com outro v
                num_testados++;
            }
        }
